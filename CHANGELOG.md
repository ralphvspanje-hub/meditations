...
## 2026-04-23 — Cursor subsection moves above Setup walkthroughs

**What happened:** the README Install block had `### Setup walkthroughs` first and `### Capturing via Cursor or VS Code` directly below it. That ordering misread what Cursor does for a new reader. The paste-in prompts all assume the user already has a working Claude Code session to paste into, and the easiest way to get that session is the Claude Code extension running inside Cursor or VS Code. Cursor is not one option among four — it is the host environment every other walkthrough runs inside of. Listing it below the walkthroughs made it look like the fourth of four candidate setups rather than the runway the other three need. This pass swaps the two subsections so the Cursor paragraph sits above the walkthroughs, and adds one sentence to the end of that paragraph making the double duty explicit: this is where you run the walkthroughs below, paste any of the prompts into a fresh Claude Code chat panel in the IDE and the walkthrough agent takes it from there.

**Changed:** swapped section order in `README.md` so `### Capturing via Cursor or VS Code` precedes `### Setup walkthroughs`; appended one sentence to the Cursor paragraph making it the host environment for the walkthroughs below.

**File:** `README.md`, `CHANGELOG.md`.

## 2026-04-23 — complementary-not-competing framing for routines and Channels

**What happened:** the README's Setup walkthroughs block listed four prompts and paste-in commands, and `phone-setup.md` opened with a "Pick your path" heading plus a "Two real paths. Pick the one whose 'what it needs' column you actually have" intro. Both surfaces read as mutually exclusive — a new reader came away thinking routines and Channels were two candidate answers to the same question, when they actually do different things in opposite directions (routines push TO the phone on a schedule; Channels lets the phone reach INTO a Marcus session on an always-on machine) and can happily run together. The README's `### Running it from your phone` subsection, directly below the walkthrough block, had also turned redundant: with the block's direction framing in place, a generic paragraph describing phone capability without the split added noise, not clarity. This pass adds one-to-two-sentence descriptions under each of the four walkthrough headers so the split is visible at a glance (cloud → phone for routines, phone → laptop for Channels), adds a one-line closer tying the two phone options together as complementary directions, deletes the now-redundant `### Running it from your phone` subsection (the Cursor subsection above it stays, carrying the recommended-default call that the block itself does not repeat), renames `phone-setup.md`'s "Pick your path" section to "Two capabilities" with a rewritten intro that states the direction split directly, and tightens the closing "The two combined" paragraph with one sentence stating these do different things and complement rather than substitute.

**Changed:** added scan-friendly one-liners under all four `### Setup walkthroughs` headers in `README.md`, added a one-line closer for the two phone options, deleted the `### Running it from your phone` subsection; renamed `## Pick your path` to `## Two capabilities` in `docs/phone-setup.md` with a rewritten direction-framed intro; adjusted the closing "The two combined" paragraph with one sentence on complementarity.

**File:** `README.md`, `docs/phone-setup.md`, `CHANGELOG.md`.
